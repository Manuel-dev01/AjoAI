...
import { AppBar, Avatar, Lrow, ConnectButton } from "@/components/ui";
...
export default function JoinCircle() {
  ...
  const circle = valid ? (input as `0x${string}`) : undefined;
  ...
  return (
    <>
      ...
      <div className="appmain">
        <div className="fld">
          <div className="fl">Circle code (address)</div>
          <input
            className="fi"
            placeholder="0x… shared by a neighbour"
            value={input}
        ...
        {circle ? (
          <Preview circle={circle} me={me} isConnected={isConnected} onJoined={() => router.push(`/app/circle/${circle}`)} />
        ) : (
          <p className="muted">Paste the circle address your circle organiser shared with you.</p>
        )}
      </div>
    </>
  );
...
function Preview({ circle, me, isConnected, onJoined }: { circle: `0x${string}`; me?: `0x${string}`; isConnected: boolean; onJoined: () => void }) {
  ...
  return (
    <>
      <div className="invite">
        ...
        <div className="nm">Circle {short(circle)}</div>
